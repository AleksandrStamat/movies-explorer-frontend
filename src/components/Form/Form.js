...
export default function Form({
  ...
  useEffect(() => {
    for (const fileKey in formErrors) {
      ...
      for (const errorKey in keyErrors) {
        if (
          keyErrors.hasOwnProperty(errorKey) &&
          keyErrors[errorKey].valid === false
        ) {
    ...
    setIsInvalid(
      Object.keys(formValues).length !== Object.keys(formErrors).length
    );
    setIsInvalid(
  ...
  return (
    <form className={className} name={name} onSubmit={handleSubmit} noValidate>
...
export function Field({ name, children }) {
  ...
  const { onChangeInput, formValues, formErrors, showErrors } = useContext(
    FormContext
  );
  ...
  const isInvalid =
    !!formErrors[name] &&
    Object.keys(formErrors[name]).some(
      (key) => formErrors[name][key].valid === false
    );

  //Определяем ключ ошибки
  const errorKey = !!formErrors[name]
    ? Object.keys(formErrors[name]).find(
        (key) => formErrors[name][key].valid === false
      )
    : "";
