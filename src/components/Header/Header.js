...
function Header(props) {
  ...
  const isAuthOrRegistr = location.pathname==="/signin" || location.pathname==="/signup";
  const isMain = location.pathname==="/";
  return (
    <>
    {(isAuthOrRegistr) ? null :
      <header className={`header ${(isMain) ? "header__blue" : "header__grid header__white"}`}>
        ...
        <>
          ...
          >
          <span className={`header__menu-button ${isOpen && "header__menu-button_close"}`}
          />
          </button>
        </>
        }
